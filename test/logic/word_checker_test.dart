...
void main() {
  test('checkWord(モンスターが存在しない)', () {
    final quizInfo = QuizInfo(answer: monsterTestList[0]);
    final inputWords = ['マ', 'リ', 'ル', 'リ'];
    final result = checkWord(monsterTestList, quizInfo, inputWords);
  ...
  test('checkWord(同音に対して完全一致・ヒット数オーバーが混ざるパターン)', () {
    ...
    final result = checkWord(monsterTestList2, quizInfo, inputWords);
    expect(result, [
